...
export default function ChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const checkLocation = async () => {
      try {
        ...
        if (status !== "granted") {
          Alert.alert("Location Required", "Please allow location to chat.");
          return;
        }
        ...
        setIsWithinRange(dist <= ALLOWED_RADIUS_KM);
      } catch (err) {
    ...
    checkLocation();
    const interval = setInterval(checkLocation, 10000); // update every 10s
    return () => clearInterval(interval);
  }, []);
  ...
  const sendMessage = () => {
    if (!message.trim() || !isWithinRange) {
      Alert.alert("Location Restricted", "You must be inside campus to chat.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <View style={!isWithinRange ? styles.blurOverlay : {}}>
        <FlatList
          ref={flatListRef}
          data={messages}
          keyExtractor={(item) => item.id}
      ...
      {!isWithinRange && (
        <Text style={styles.restrictedBadge}>
          ❌ Outside allowed 3km campus area
        </Text>
      )}
    </KeyboardAvoidingView>
  );
