...
const ALLOWED_LON = 80.2363;
const ALLOWED_RADIUS = 3000; // 3 km
...
export default function ChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const setup = async () => {
      ...
      if (community.created_by === userId) {
        await joinAsAdmin(community.id, userId); // Admin bypass ✅
      } else if (status !== "approved") {
        Alert.alert("❌ Access Denied", "You are not a community member.");
        return navigation.goBack();
      }
      ...
      if (locationStatus !== "granted") {
        Alert.alert("⚠️ Location Required", "Enable location to chat here!");
        setAllowed(false);
      }
      ...
      if (distance > ALLOWED_RADIUS && community.created_by !== userId) {
        setAllowed(false);
      }
  ...
  const sendMessage = () => {
    if (!message.trim() || !allowed) return;
    ...
    >
      ...
      {!allowed && (
        <Text style={styles.restrictedText}>
          ❌ You must be within 3 km of the permitted zone to chat.
        </Text>
      )}

      <View style={!allowed ? styles.blur : null}>
        <FlatList
          ref={flatListRef}
          data={messages}
          keyExtractor={(item) => item.id}
      ...
      <View style={styles.inputRow}>
        ...
        />
        <TouchableOpacity
          style={[styles.send, !allowed && styles.disabledBtn]}
          onPress={sendMessage}
          disabled={!allowed}
        >
          <Text style={{ color: "#fff" }}>Send</Text>
