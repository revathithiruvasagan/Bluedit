...
//       <Text style={styles.distanceText}>📍 You are {distance} km away</Text>
...
// ✅ Allowed Location (Hardcoded)
const ALLOWED_LAT = 13.0111;
const ALLOWED_LNG = 80.2363;
const ALLOWED_RADIUS_KM = 3;
...
export default function ChatScreen({ route, navigation }) {
  ...
  useEffect(() => {
    const setup = async () => {
      ...
      if (community.created_by === userId) {
        ...
        Alert.alert("Pending Approval", "Wait for admin approval!");
        return navigation.goBack();
      } else if (status !== "approved") {
        Alert.alert("Join Required", "Request to join first!");
        return navigation.goBack();
      }
  ...
  const sendMessage = () => {
    if (!message.trim()) return;
    if (!isWithinRange)
      return Alert.alert("Restricted", "Enter campus to chat!");
  ...
  return (
    ...
    >
      ...
      <Text style={styles.distanceText}>
        📍 {distance} km away {isWithinRange ? "✅" : "❌"}
      </Text>
      ...
      <View style={{ flex: 1 }}>
        ...
        {!isWithinRange && (
          <Animated.View style={[styles.blurWrapper, { opacity: blurAnim }]}>
            ...
            <View style={styles.lockContainer}>
              <Text style={styles.restrictedText}>Campus Restricted Chat</Text>
              <Text style={styles.restrictedSubText}>
                Move closer to the allowed location 📍
              </Text>
            </View>
          </Animated.View>
        )}
...
const styles = StyleSheet.create({
  ...
  darkOverlay: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: "rgba(0,0,0,0.85)", // ✅ Almost fully dark
  },
